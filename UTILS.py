...
def generate_labels():
    for name in os.listdir('F:/dataset/UNDERALL/2019origin/train_data/annotations/'):
        image = cv2.imread('F:/dataset/UNDERALL/2019origin/train_data/images/'+name[:-4]+'.jpg')
        try:
            dw = 1. / float(image.shape[1])
            dh = 1. / float(image.shape[0])
        except:
            print(name[:-4]+'.jpg')
        RR = open('F:/dataset/UNDERALL/2019origin/train_data/annotations/' + name, 'r')
        yolo = open('F:/dataset/UNDERALL/2019yolo/train/labels/' + name, 'w')
        for line in RR.readlines():
            cls = int(line.strip().split(',')[5]) - 1
            x_center = (2 * float(line.split(',')[0]) + float(line.split(',')[2])) / 2.0 -1
            y_center = (2 * float(line.split(',')[1]) + float(line.split(',')[3])) / 2.0 -1
...
def generate_labelpath():
    train_txt = open('F:/dataset/UNDERALL/2019yolo/train.txt', 'w')
    for name in os.listdir('F:/dataset/UNDERALL/2019yolo/train/images/'):
        train_txt.write('../2019yolo/train/images/'+name+'\n')

    val_txt = open('F:/dataset/UNDERALL/2019yolo/val.txt', 'w')
    for name in os.listdir('F:/dataset/UNDERALL/2019yolo/val/images/'):
        val_txt.write('../2019yolo/val/images/'+name+'\n')

def remove_empty():
    image_list = os.listdir('F:/dataset/UNDERALL/2019yolo/train/images/')
    for name in os.listdir('F:/dataset/UNDERALL/2019yolo/train/labels/'):
        if name[:-4]+'.jpg' not in image_list:
            os.remove('F:/dataset/UNDERALL/2019yolo/train/labels/'+name)
...
def compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus):
    ...
    for label_file in os.listdir('F:/dataset/UNDERALL/2019yolo/train/labels/'):
        f = open('F:/dataset/UNDERALL/2019yolo/train/labels/'+label_file)
        for line in f:
...
if __name__ == '__main__':
    # generate_labels()
    # remove_empty()
    # remove_empty_file('F:/dataset/UNDERALL/2019yolo/train/labels/')
